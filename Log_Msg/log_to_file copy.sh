  ...
  for dir in "${dirs[@]}"; do
    ...
    for file in "${files[@]}"; do
      ...
      { : > "$full_path"; } 2>/dev/null || {
        errors_map["$full_path"]="Create File failed | no Permission"
        continue
      }
      ...
      for ((i=total-1; i>0; i--)); do
        # Skriptpfad, nur wenn anders
        if [[ "${BASH_SOURCE[i]}" != "${BASH_SOURCE[i-1]}" ]]; then
          entries+=("$(realpath "${BASH_SOURCE[i]}")")
        fi
        # Funktionen außer log_call_chain
        if [[ "${FUNCNAME[i]}" != "log_call_chain" ]]; then
          entries+=("${FUNCNAME[i]}")
        fi
      done
  ...
  if [[ ${#other_logs[@]} -gt 0 ]]; then
    for path in "${other_logs[@]}"; do
      ...
      for file in "${other_logs[@]}"; do
        echo "|  $file" >> "$path"
        # Fehler direkt unter jeder Datei ausgeben
