  ...
  for dir in "${valid_dirs[@]}"; do
    ...
    for file in "$dir"/*.ini; do
      ...
      if [[ -z "$lang_code" || -z "$lang_name" || -z "$name" || "$name" != "$NAME_KEY" ]]; then
        # log_message "WARN" "INI '$file' hat fehlende meta-Werte oder falsches NAME_KEY, wird übersprungen"
        continue
      fi
  ...
  while IFS= read -r line; do
    ...
    case "$line" in
      ...
      *=*)
        local key="${line%%=*}"
        local value="${line#*=}"
        [[ -n "$current_section" ]] && config["${current_section}.${key}"]="$value" || config["$key"]="$value"
        if [[ -n "$current_section" && "$key" != "type" ]]; then
          menu_order["$current_section"]="${menu_order[$current_section]:-} $key"
        fi
        ;;
    esac
  done < "$CURRENT_LANG_FILE"
